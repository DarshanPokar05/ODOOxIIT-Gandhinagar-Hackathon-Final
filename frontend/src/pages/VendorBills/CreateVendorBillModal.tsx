...
const CreateVendorBillModal: React.FC<Props> = ({ isOpen, onClose, onBillCreated }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const purchaseProducts = productsRes.data.filter((p: Product) => p.type_purchase && p.cost_price > 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/vendor-bills', payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      const errorMsg = error.response?.data?.message || error.message || 'Failed to create vendor bill';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 style={{ margin: 0, fontSize: '24px', fontWeight: '600' }}>Create Vendor Bill</h2>
          <button 
            type="button"
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Bill Date</label>
              <input
                type="date"
          ...
          <div style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '600' }}>Bill Lines</h3>
              <button
                type="button"
                onClick={addLine}
                style={{
